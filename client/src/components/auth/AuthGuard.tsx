...
export const AuthGuard = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    if (!user && isAuthenticated) {
      // Show "waking up" hint after 5 s
      const slowTimer = setTimeout(() => setSlow(true), 5_000);

      // Hard bail-out at 35 s: clear the stale token so the normal
      // !isAuthenticated render path redirects to /login cleanly.
      // (The 30 s axios timeout should fire first, but this is a safety net.)
