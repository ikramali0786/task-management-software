...
import {
  ...
  Plus, ChevronDown, ChevronRight, BarChart2, Bot, Activity, CalendarDays, User, Target,
} from 'lucide-react';
...
export const Sidebar = () => {
  ...
  useEffect(() => {
    if (sidebarCollapsed) setShowTeamMenu(false);
  }, [sidebarCollapsed]);
      ...
      <AnimatePresence>
        {sidebarOpen && (
          ...
          >
            {/* Logo */}
            <div className={cn(
              'flex items-center gap-2.5 px-5 py-5 flex-shrink-0',
              sidebarCollapsed && 'justify-center px-0'
            )}>
              <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl gradient-brand">
            ...
            {/* Team Switcher */}
            <div className={cn('px-3 flex-shrink-0', sidebarCollapsed && 'px-1.5')}>
              {sidebarCollapsed ? (
                /* Collapsed: show team initial only */
                <div className="flex justify-center">
                  <div
                    title={activeTeam?.name || 'Select Team'}
                    className="flex h-9 w-9 items-center justify-center rounded-xl bg-brand-500 text-xs font-bold text-white cursor-default"
                  >
                    {activeTeam ? activeTeam.name[0].toUpperCase() : '?'}
                  </div>
                </div>
              ) : (
                /* Full: full team switcher */
                <>
                  <button
