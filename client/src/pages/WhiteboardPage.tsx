...
import { exportPNG, exportSVG, exportPDF, copyPNGToClipboard } from '@/lib/whiteboardExport';
import { TASK_STATUSES, PRIORITY_CONFIG } from '@/types';
import { cn } from '@/lib/utils';
import type {
  Tool, Align, Style, Base, ShapeType, Shape, Txt, PathEl, LineEl, ImageEl, TaskEl, Connector, El, GridMode, Peer,
...
export const WhiteboardPage = () => {
  ...
  const shareUrl = currentBoard?.publicToken ? `${window.location.origin}/wb/${currentBoard.publicToken}` : '';
  const toggleShare = async () => {
    if (!boardId) return; setShareBusy(true);
    try {
      if (currentBoard?.isPublic) { const r = await whiteboardService.disableShare(boardId); setBoards((l) => l.map((b) => b._id === boardId ? { ...b, isPublic: false, publicToken: r.publicToken } : b)); }
      else { const r = await whiteboardService.enableShare(boardId); setBoards((l) => l.map((b) => b._id === boardId ? { ...b, isPublic: true, publicToken: r.publicToken } : b)); }
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] flex-col bg-slate-50 dark:bg-slate-950">
      ...
      <div className="z-10 flex flex-wrap items-center gap-1.5 border-b border-slate-200 bg-white/80 px-4 py-2 backdrop-blur dark:border-slate-800 dark:bg-slate-900/80">
        ...
        <div className="mx-1 h-5 w-px bg-slate-200 dark:bg-slate-700" />
        <div className="relative">
          <button onClick={() => setExportMenu((v) => !v)} title="Export" aria-label="Export" className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800"><Download className="h-4 w-4" /></button>
          {exportMenu && (
            <>
              ...
              <div className="absolute left-0 top-full z-30 mt-1.5 w-52 overflow-hidden rounded-xl border border-slate-200 bg-white py-1 text-sm shadow-xl dark:border-slate-700 dark:bg-slate-800">
                ...
                <div className="my-1 border-t border-slate-100 dark:border-slate-700/60" />
                <button onClick={() => runExport('copy')} className="flex w-full items-center gap-2.5 px-3 py-2 text-left text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-700"><Copy className="h-3.5 w-3.5" /> Copy as image</button>
              </div>
            </>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setShareOpen(false)}>
          <div className="w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3 dark:border-slate-800">
              <span className="flex items-center gap-2 text-base font-semibold text-slate-800 dark:text-slate-100"><Share2 className="h-5 w-5 text-brand-500" /> Share “{boardName}”</span>
            ...
            <div className="space-y-4 p-5">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <div className="text-xs text-slate-400">Anyone with the link can view this board — no account needed.</div>
                </div>
                <button onClick={toggleShare} disabled={shareBusy} role="switch" aria-checked={!!currentBoard?.isPublic} className={cn('relative h-6 w-11 shrink-0 rounded-full transition-colors', currentBoard?.isPublic ? 'bg-brand-500' : 'bg-slate-300 dark:bg-slate-600', shareBusy && 'opacity-60')}>
                  <span className={cn('absolute top-0.5 h-5 w-5 rounded-full bg-white transition-transform', currentBoard?.isPublic ? 'translate-x-5' : 'translate-x-0.5')} />
                </button>
              </div>
              {currentBoard?.isPublic && shareUrl ? (
                <div className="space-y-3">
                  <div>
                    <label className="mb-1 block text-xs font-medium text-slate-500">Share link</label>
                  ...
                  <div>
                    <label className="mb-1 block text-xs font-medium text-slate-500">Embed (iframe)</label>
                    <div className="flex gap-2">
                      <input readOnly value={`<iframe src="${shareUrl}?embed=1" width="800" height="600" frameborder="0"></iframe>`} className="min-w-0 flex-1 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1.5 font-mono text-[11px] text-slate-600 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300" />
                      <button onClick={() => copyText(`<iframe src="${shareUrl}?embed=1" width="800" height="600" frameborder="0"></iframe>`, 'Embed code copied')} className="flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"><Copy className="h-3.5 w-3.5" /></button>
                    </div>
                  </div>
                  <a href={shareUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-xs font-medium text-brand-600 hover:underline dark:text-brand-400"><FolderOpen className="h-3.5 w-3.5" /> Open the public view</a>
                </div>
              ) : (
                <p className="rounded-lg bg-slate-50 px-3 py-2.5 text-xs text-slate-400 dark:bg-slate-800">Turn on the public link to get a shareable URL and an iframe embed snippet. The shared view is always read-only.</p>
              )}
            </div>
          </div>
        </div>
